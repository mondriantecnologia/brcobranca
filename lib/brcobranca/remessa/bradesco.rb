      ...
      def corpo_arquivo_repasse
        ...
        self.objeto.itens.each do |item_repasse|
          if item_repasse.valor >= 0.01
            ...
            ## trata formatacao do cpf e cnpj para arquivo
            vcpf = item_repasse.cartorio.dados_cartorario.cpf_cnpj.gsub(/[^0-9]/,'')
            if vcpf.size == 11
              vcpf = vcpf[0,9] + '0000' + vcpf[9,2]
            end   
            linha = "1"
            linha << item_repasse.cartorio.dados_cartorario.tipo_de_inscricao.to_s # 002 A 002 - Tipo de inscricao do Fornecedor - 1- CPF, 2- CNPJ, 3-OUtros
            linha << vcpf.rjust(15,'0') #003 A 017 - CPF/CNPJ fornecedor
            linha << I18n.transliterate(item_repasse.cartorio.dados_cartorario.nome)[0,30].gsub('º',' ').gsub('°',' ').upcase.ljust(30) #018 A 047 - Nome do fornecedor
            linha << I18n.transliterate(item_repasse.cartorio.dados_cartorario.endereco)[0,40].gsub('º',' ').gsub('°',' ').upcase.ljust(40,' ') # 048 A 087 Endereco do fornecedor
            linha << item_repasse.cartorio.dados_cartorario.cep.gsub('.','').gsub('-','') #088 A 095 -  Cep do fornecedor
            linha << item_repasse.cartorio.dados_cartorario.codigo_banco.rjust(3,'0') #096 A 098 - Codigo do banco do fornecedor
            linha << item_repasse.cartorio.dados_cartorario.codigo_agencia.rjust(5,'0') # 099 A 103 - Codigo da agencia do fornecedor
            linha << item_repasse.cartorio.dados_cartorario.digito_agencia.ljust(1,' ') # 104 A 104 - digito da agencia do fornecedor
            linha << item_repasse.cartorio.dados_cartorario.conta_corrente.rjust(13,'0') # 105 A 117 - conta ocrrente do fornecedor
            linha << item_repasse.cartorio.dados_cartorario.digito_conta_corrente.ljust(2,' ') #118 A 119 - digito da conta corrente do fornecedor
            ...
            linha << '0'.rjust(10,'0') #252 A 261 - Numero nota fiscal - fixo 0
            linha << '  ' #262 A 263 - serie do documento - opicional 
            linha << (item_repasse.cartorio.dados_cartorario.codigo_banco == '237' ? '05' : '03' ) #264 A 265 - modalidade de pagamento - 01 = transf, 03 = doc
            ...
            linha << ' ' # 478 A 478 reserva - fixo branco
            linha << item_repasse.cartorio.dados_cartorario.tipo_conta.to_s #479 A 479 - Tipo de conta do fornecedor - 1 = conta corrente, 2= conta poupanca
            linha << self.parametros.conta.rjust(7,'0') #480 A 486 - Conta complementar - Fixo 0
            linha << ''.ljust(8,' ') # 487 A 494 - fixo branco
            linha << numero_de_registros.to_s.rjust(6,'0')  # 495 a 500 num sequencial do registro
            linha << "\n"
